...
def _grab_tile(lat, lon, zoom, maptype, _TILESIZE, sleeptime):
    ...
    #mapscache_path =  'mapscache'
    mapscache_path  = os.path.join(os.path.dirname(os.path.realpath(__file__)), "mapscache")
    filename = os.path.join(mapscache_path, (('%f_%f_%d_%s_%d_%d' % specs) + '.jpg'))
    tile = None
    ...
    if os.path.isfile(filename):
        ...
        if not os.path.exists(mapscache_path):
            os.mkdir(mapscache_path)
        tile.save(filename)
